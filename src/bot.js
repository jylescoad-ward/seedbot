...
//Developer Commands
function resetBot(channel){channel.send('Resetting...').then(msg=>client.destroy()).then(()=>client.login(config.token))}client.on('message',async message=>{if(message.author.bot)return;if(message.content.indexOf(config.devprefix)!==0)return;const args=message.content.slice(config.devprefix.length).trim().split(/ +/g);const devcommand=args.shift().toLowerCase();const announcementschannel=client.channels.find('name','announcements');const generalchannel=client.channels.find('name','general');if(devcommand==='getipv4'){if(message.author.id===ownerID){publicIp.v4().then(ip=>{message.author.send('**Global IPv4:**\n*'+ip+'*')})}}if(devcommand==='getipv6'){if(message.author.id===ownerID){publicIp.v6().then(ip=>{message.author.send('**Global IPv6:**\n*'+ip+'*')})}}if(devcommand==='announce'){let message=args.slice(0).join(" ")let array=client.channels.array().sort();if(message.author.id===ownerID){client.channels.get(array).send(message)}else{message.reply('you do not have permissions to use this devcommand,\n so ***a s c e n d*** to the 4th ***d i m e n s i o n***')}}if(devcommand==='serverlist'){if(message.author.id===config.ownerId){message.author.send("Number of Available Servers: "+client.guilds.size);var list=client.guilds.array().sort();message.author.send("Available Servers: "+list);signal.info("An Owner executed s!srvrs")}else{message.reply('you do not have permissions to use this devcommand,\n so ***a s c e n d*** to the 4th ***d i m e n s i o n***')}}if(devcommand==='usercount'){if(message.author.id===ownerID){message.author.send("Number of Users: "+client.users.size)}else{message.reply('you do not have permissions to use this devcommand,\n so ***a s c e n d*** to the 4th ***d i m e n s i o n***')}}if(devcommand==='channels'){if(message.author.id==='230485481773596672'){message.author.send("Number of Available Channels: "+client.channels.size);var list=client.channels.array().sort();signal.command("An Owner executed s!channels")}else{message.reply('you do not have permissions to use this devcommand,\n so ***a s c e n d*** to the 4th ***d i m e n s i o n***')}}if(['eval','exec'].some(arx=>devcommand==arx)){let code=args.slice(0).join(" ");if(message.author.id===ownerID){if(!code)return message.channel.send('No code provided!');this.client=bot;const evaled={},logs=[];const token=this.client.token.split('').join('[^]{0,2}'),rev=this.client.token.split('').reverse().join('[^]{0,2}'),tokenRegex=new RegExp(`${token}|${rev}`,'g'),cba='```js\n',cb='```';const print=(...a)=>{const cleaned=a.map(obj=>{if(typeof o!=='string')obj=util.inspect(obj,{depth:1});return obj.replace(tokenRegex,'Nice try getting a token.')});if(!evaled.output){logs.push(...cleaned);return}evaled.output+=evaled.output.endsWith('\n')?cleaned.join(' '):`\n${cleaned.join(' ')}`;const title=evaled.errored?'☠\u2000**Error**':'📤\u2000**Output**';if(evaled.output.length+code.length>1900)evaled.output='Output too long.';var emb=new RichEmbed().setColor('GREEN').addField(`📥\u2000**Input**`,`${cba}js`+code+cb).addField(`${title}`,`${cba}js`+evaled.output+cb).setTimestamp();evaled.message.edit("",emb)};try{let output=eval(code);if(output&&typeof output.then==='function')output=await output;if(typeof output!=='string')output=util.inspect(output,{depth:0});output=`${logs.join('\n')}\n${logs.length && output === 'undefined' ? '' : output}`;output=output.replace(tokenRegex,'Nice try getting a token.');if(output.length+code.length>1900)output='Output too long.';var emb=new Discord.RichEmbed().setColor('GREEN').addField(`📥\u2000**Input**`,`${cba}`+code+cb).addField(`📤\u2000**Output**`,`${cba}`+output+cb).setTimestamp();const sent=await message.channel.send("",emb);evaled.message=sent;evaled.errored=!1;evaled.output=output;return sent}catch(err){console.error(err);let error=err;error=error.toString();error=`${logs.join('\n')}\n${logs.length && error === 'undefined' ? '' : error}`;error=error.replace(tokenRegex,'Nice try getting a token.');var emb=new Discord.RichEmbed().setColor('RED').addField(`📥\u2000**Input**`,`${cba}`+code+cb).addField(`☠\u2000**Error**`,`${cba}`+error+cb).setTimestamp();const sent=await message.channel.send("",emb);evaled.message=sent;evaled.errored=!0;evaled.output=error;return sent}}else{message.reply('you do not have permissions to use this devcommand,\n so ***a s c e n d*** to the 4th ***d i m e n s i o n***')}}if(devcommand==="restart"){if(message.author.id===ownerID){message.channel.send('Bot it now Restarting. Good Night :first_quarter_moon_with_face: :bed: ');client.user.setActivity('Bot is Restarting...');resetBot(message.channel);break}else{message.reply('you do not have permissions to use this devcommand,\n so ***a s c e n d*** to the 4th ***d i m e n s i o n***')}}if(devcommand==='refreshrpc'){if(message.author.id===ownerID){message.channel.send('Rich Presence Refreshed!');client.user.setActivity(packageJSON.homepage+'// Serving '+client.users.size+' players')}else{message.reply('you do not have permissions to use this devcommand,\n so ***a s c e n d*** to the 4th ***d i m e n s i o n***')}}if(devcommand==='rpc'){var game=args.slice(0).join(" ");if(message.author.id==='230485481773596672'){if(game==='reset'){client.user.setActivity(packageJSON.homepage+'// Serving '+client.users.size+' clients');message.channel.send('***Rich Presence Has Been Reset***');signal.info("A Owner executed s!rp reset")}else{client.user.setActivity(game+' // '+packageJSON.homepage+'// Serving '+client.users.size+' clients');message.channel.send('***Rich Presence Status Updated To:*** \n'+"`"+game+"`");signal.info("A Owner executed s!rp "+game+", game set to \n"+game)}}else{message.reply('you do not have permissions to use this devcommand,\n so ***a s c e n d*** to the 4th ***d i m e n s i o n***')}}if(devcommand==='deathnote'){if(message.author.id===ownerID){clients.channels.get(deathnotereciever.package).send(`Hey... I'm dead.\nSorry if this news is abrupt but i have. life has been really fucking tough and I want you to deliver the news to my mother...\n\nI give you premission to use and take all of my electronics ***except*** the mac mini that is inside of my desk. if you do take it please read in the zipped file that is attached how to keep me running from the grave.\n\n here is how to unlock my computers.\n https://storage.googleapis.com/file-asia/private/for_you.zip\n\n\nSay to amber that I love her and dont make this a big mess please.`);message.reply('have a good afterlife :heart:')}else{message.reply('go fuck youself')}}})
