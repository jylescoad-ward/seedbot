...
client.on("message", async message => {
  ...
  //if(command === "unbancreator") {
    // creator id: 230485481773596672
    // Unban a user by ID (or with a user/guild member object)
  //  guild.unban('230485481773596672')
  //  .then(user => console.log(`Unbanned ${user.username} from ${guild}`))
  //  .catch(console.error);
  //  console.log(.reply + "ran the unbancreator command");
  //}
  if (command === 'servers') {
  ...
  if (command == "help") {
    return message.reply("For the command List then click on this link \n http://bot.jyles.pw/command.html");
  }
  if (command === "setup") {
    message.reply("Create two roles \n One role with the name of ``seedmod`` and rename the other role ``seedadmin`` \n Assign theese roles to your mods/admins so they can use the kick/ban/purge commands");

  }
  if (command === "clearchat") {

    const deleteCoutn = parseInt(args[0], 10);

    const fetched = await message.channel.fetchMessages({limit: '255'});
    message.channel.bulkeDelete(fetched)
      .catch(error => message.reply(`Couldn't delete messages because of: ${error}`))
  }
});
